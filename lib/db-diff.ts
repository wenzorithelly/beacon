...
export function diffDraftTables(
  ...
  for (const d of draft) {
    ...
    if (!r) {
      out.set(d.id, { status: "added", changes: ["new table"] });
      continue;
    }
    const changes = diffColumns(r.columns, d.columns);
    out.set(d.id, { status: changes.length ? "modified" : "unchanged", changes });
  }
  return out;
}

function diffColumns(real: ReadonlyArray<ColLike>, draft: ReadonlyArray<ColLike>): string[] {
  ...
  for (const c of draft) {
    ...
    if (edits.length) changes.push(`~ column ${c.name}: ${edits.join(", ")}`);
  }
  for (const c of real) {
...
export function diffDraftEndpoints(
  ...
  const out = new Map<string, NodeDiff>();
  for (const d of draft) {
    out.set(d.id, realKeys.has(key(d)) ? { status: "unchanged", changes: [] } : { status: "added", changes: ["new endpoint"] });
  }
  return out;
}
