...
const MAX_BAND_W = 6 * TABLE_COL_WIDTH;
export const UNATTACHED_GROUP = "Unattached";

export const domainKey = (d: string | null | undefined): string => (d ?? "").trim() || "—";

/** The table an endpoint touches most (most usage rows); ties break alphabetically by table
 *  name so the choice is stable. Unknown tableIds are ignored; null when nothing resolves. */
export function primaryTableFor(
  ...
  for (const [id, n] of counts) {
    if (
      ...
      n > counts.get(best)! ||
      (n === counts.get(best)! &&
        tableNameById.get(id)!.localeCompare(tableNameById.get(best)!) < 0)
    ) {
      best = id;
    }
  }
...
export function computeDbBoardLayout(
  ...
  for (const e of [...endpoints].sort(epSort)) {
    const pid = primaryTableFor(e, tableNameById);
    if (pid) {
  ...
  for (const d of domainOrder) {
    ...
    const cols = Math.max(1, Math.min(4, Math.ceil(Math.sqrt(members.length * 1.7))));
...
export function nextTableSlot(
  ...
  const tableNameById = new Map(existingTables.map((t) => [t.id, t.name]));
  ...
  const cols = Math.min(4, Math.max(Math.max(...ks) + 1, Math.ceil(Math.sqrt(members.length * 1.7))));
...
export function nextEndpointDock(
  ...
  const nameById = tableNameById ?? new Map(tables.map((t) => [t.id, t.name]));
  const pid = primaryTableFor(ep, nameById);
  const t = pid ? tables.find((x) => x.id === pid) : undefined;
  if (t) {
    const docked = endpoints.filter((e) => primaryTableFor(e, nameById) === t.id);
