...
// `beacon_describe_feature` call keyed by id — which is what collapses the old N-calls-plus-
// disambiguation-retries close-out into a single round-trip.
export function approvedFeaturesContext(features: ApprovedFeature[] | undefined): string {
  ...
  return (
    ...
    lines.join("\n") +
    "\n\nWhen the work ships, register them in ONE call: `beacon_describe_feature` with a " +
