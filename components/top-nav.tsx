...
export function TopNav({ repo }: { repo?: string }) {
  ...
  // Carry this tab's workspace through nav so clicking Plans / Map / Settings keeps the tab pinned
  // instead of dropping ?ws and falling back to the shared cookie. Read after mount (window-only) so
  // the initial client render matches SSR (no ?ws), then fills in.
  ...
  return (
    <header data-app-nav className="pointer-events-none fixed left-3 top-3 z-30">
