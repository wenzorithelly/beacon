...
describe("planReconcile (full current set)", () => {
  ...
  it("pushes a Beacon-edited card that is still in the set", () => {
    const n = local({ externalUpdatedAt: 100, externalSyncedAt: 100, updatedAt: 300 });
    const i = issue({ updatedAt: 100 }); // Linear unchanged since last mirror
    expect(planReconcile([n], [i])).toEqual([{ action: "push", node: n, issue: i }]);
  });

  it("removes a local card that is no longer in the set (left scope / unassigned / closed)", () => {
    const n = local({ updatedAt: 999, externalSyncedAt: 100 }); // even a pending local edit is dropped
    expect(planReconcile([n], [])).toEqual([{ action: "remove", node: n }]);
  });
  ...
  it("handles a mix in one pass", () => {
    ...
    const gone = local({ id: "q", externalId: "qe", updatedAt: 900, externalSyncedAt: 1 }); // not in the set
    const plan = planReconcile([pulling, gone], [fresh, pullIssue]);
    expect(plan).toContainEqual({ action: "create", issue: fresh });
    expect(plan).toContainEqual({ action: "pull", node: pulling, issue: pullIssue });
    expect(plan).toContainEqual({ action: "remove", node: gone });
